...
const tenantsRequests = {
  ...
  listAllOffices: async (data: {
    ...
    return await axiosInstance.post(
      buildApiUrl(`${TENANT_BASE_URL}/users/cashboxes/list`),
      { tenantUId: data.tenantUId, userUId: data.userUId }, 
      { headers: getTenantHeaders() }
    )
  },
  ...
  removeTenantCashBoxFromUser: async (data: {
    tenantUId: string
    userUId: string
    cashboxUId: string
  }): Promise<AxiosResponse<{ message: string }>> => {
    return await axiosInstance.delete(
      buildApiUrl(`${TENANT_BASE_URL}/users/cashboxes/remove`),
      {
        data,
        headers: getTenantHeaders(),
      }
