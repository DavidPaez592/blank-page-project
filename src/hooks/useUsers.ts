...
export const useGetUserPermissions = (): IUseUserGetPermissionsResult => {
  ...
  }, [UsersState.currentPermissionsRoleUId.value])
  return {
    loading,
  }
}
...
interface IUseUserTenantsResult {
  handleGetUserTenants: (userUId: string) => void
  loading: Signal<{
    get: boolean
    update: boolean
  }>
...
export const useUserTenants = (): IUseUserTenantsResult => {
  const loading = useSignal({
    get: false,
    update: false,
  })

  const handleGetUserTenants = (userUId: string): void => {
    loading.value = { ...loading.peek(), get: true }
    usersStateActions
      .getUserTenants(userUId)
      .catch((error: any) => {
        createNotification(
          'error',
          'Error',
          'Error obteniendo los tenants del usuario'
        )
        consoleLog.error('GETUSERTENANTS::ERROR', error)
      })
      .finally(() => {
        loading.value = { ...loading.peek(), get: false }
      })
